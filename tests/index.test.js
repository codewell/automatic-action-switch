const automaticActionSwitch = require("../lib/prod");
...
test("Set foo", () => {
  expect(defaultActionSwitch(state, { type: SET_FOO, payload: 1 })).toEqual({
    foo: 1,
  });
});
...
test("Update foo", () => {
  expect(
    defaultActionSwitch(state, {
      type: UPDATE_FOO,
      payload: { two: 2, three: 3 },
    }),
...
test("Remove foo", () => {
  expect(defaultActionSwitch(state, { type: REMOVE_FOO })).toEqual({});
});
...
test("Custom prefix set, wrong type", () => {
  expect(customActionSwitch(state, { type: SET_FOO, payload: 1 })).toEqual(
    state,
  );
});

test("Custom prefix set", () => {
  expect(customActionSwitch(state, { type: AUTOSET_FOO, payload: 1 })).toEqual({
    foo: 1,
  });
});
...
test("Custom prefix remove, wrong type", () => {
  expect(customActionSwitch(state, { type: REMOVE_FOO })).toEqual(state);
});

test("Custom prefix remove", () => {
  expect(customActionSwitch(state, { type: AUTOREMOVE_FOO })).toEqual({});
});
 ...
 */
test("Custom action", () => {
  expect(customActionSwitch(state, { type: CUSTOM_FOO })).toEqual("customFoo");
});
